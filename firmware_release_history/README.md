...
4. Flash by USB or OTA in a supervised beta test. Devices on beta 14 or earlier take the new firmware only by USB (`app.uf2` carries the OTA bootloader); never send the new `app.bin` through a pre-bootloader device's OTA.
